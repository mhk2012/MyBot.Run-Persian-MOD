			...
			For $i = 0 To UBound($aResult) - 1
				...
				If $aResult[$i][1] > 0 Then
					...
					Else
						...
						$aResult[$i][4] = -1
					EndIf
					If $aResult[$i][4] <= 10 Or Not $g_abChkExtendedAttackBar[$g_iMatchMode] Or Not $SWIPE = "" Then ; ExtendedAttackBar - Persian MOD (#-22)
...
Func SlotAttack($PosX, $CheckSlot12, $CheckSlotwHero, $SRIGHT, $Troop)
	...
	For $i = 0 To 12
		If $PosX >= $iStartOffset + ($i * 73) And $PosX < (($iStartOffset + 73 + $iStartOffsetHero) + ($i * 73)) Then
			...
			If $g_bDebugSetlog = 1 Then SetDebugLog("Slot: " & $i & " | $x > " & $iStartOffset + ($i * 73) & " and $x < " & ($iStartOffset + 73 + $iStartOffsetHero) + ($i * 73))
			If $g_bDebugSetlog = 1 Then SetDebugLog("Slot: " & $i & " | $PosX: " & $PosX & " |  OCR x position: " & $Slottemp[0] & " | OCR Slot: " & $Slottemp[1])
			Return $Slottemp
		EndIf
		If Not $g_bRunState Then Return
	Next
